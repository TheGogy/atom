...
void format_cp_compact(Value v, char* buffer, const Position& pos) {
    ...
    int cp = std::abs(Uci::toCentipawns(v, pos));
    if (cp >= 10000)
    {
        buffer[1] = '0' + cp / 10000;
        cp %= 10000;
...
void format_cp_aligned_dot(Value v, std::stringstream& stream, const Position& pos) {
    ...
    const double pawns = std::abs(0.01 * Uci::toCentipawns(v, pos));
...
std::string
trace(Position& pos, const NNUE::Networks& networks, NNUE::AccumulatorCaches& caches) {
    ...
    base                    = pos.getSideToMove() == WHITE ? base : -base;

    for (File f = FILE_A; f <= FILE_H; ++f)
        for (Rank r = RANK_1; r <= RANK_8; ++r)
        {
            Square sq = createSquare(f, r);
            Piece  pc = pos.getPieceAt(sq);
            Value  v  = VALUE_NONE;

            if (pc != NO_PIECE && typeOf(pc) != KING)
            {
                ...
                eval                       = pos.getSideToMove() == WHITE ? eval : -eval;
                v                          = base - eval;
    ...
    ss << " NNUE network contributions "
       << (pos.getSideToMove() == WHITE ? "(White to move)" : "(Black to move)") << std::endl
       << "+------------+------------+------------+------------+\n"
